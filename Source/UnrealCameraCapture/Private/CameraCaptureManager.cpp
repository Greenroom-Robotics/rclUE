...
void UCameraCaptureManagerComponent::SaveCapture(FRenderRequestStruct* NextRenderRequest)
{
  ...
  if (UsePNG)
  {
    ...
    imageWrapper->SetRaw(NextRenderRequest->Image.GetData(), NextRenderRequest->Image.GetAllocatedSize(), FrameWidth,
                         FrameHeight, ERGBFormat::BGRA, 8);
...
void UCameraCaptureManagerComponent::CaptureNonBlocking()
{
  ...
  FReadSurfaceContext readSurfaceContext = {
    ...
    FReadSurfaceDataFlags(RCM_UNorm, CubeFace_MAX), renderRequest->CaptureTime
  };
  ...
  ([readSurfaceContext](FRHICommandListImmediate& RHICmdList) {
    ...
    RHICmdList.ReadSurfaceData(readSurfaceContext.SrcRenderTarget->GetRenderTargetTexture(), readSurfaceContext.Rect,
                               *readSurfaceContext.OutData, readSurfaceContext.Flags);
...
void UCameraCaptureManagerComponent::SpawnSegmentationCaptureComponent(ASceneCapture2D* ColorCapture){
    ...
    ASceneCapture2D* newSegmentationCapture = (ASceneCapture2D*)
GetWorld()->SpawnActor<ASceneCapture2D>(ASceneCapture2D::StaticClass()); if(!newSegmentationCapture){ // nullptr check
