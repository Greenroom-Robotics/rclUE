...
void UROS2ActionServer::InitializeActionComponent(const UROS2QoS QoS)
{
  ...
  server_opt.goal_service_qos = QoSProfiles_LUT[QoS];
  server_opt.result_service_qos = QoSProfiles_LUT[QoS];
  server_opt.cancel_service_qos = QoSProfiles_LUT[QoS];
  server_opt.feedback_topic_qos = QoSProfiles_LUT[QoS];
  server_opt.status_topic_qos = QoSProfiles_LUT[QoS];
  ...
  rcl_ret_t rc = rcl_action_server_init(&server, NodeSubsystem->GetRCLNode(), &ros_clock, action_type_support,
                                        TCHAR_TO_UTF8(*ActionName), &server_opt);
...
void UROS2ActionServer::ProcessReady(rcl_wait_set_t* wait_set)
{
  ...
  RCSOFTCHECK(rcl_action_server_wait_set_get_entities_ready(wait_set, &server, &IsReady[0], &IsReady[1], &IsReady[2],
                                                            &IsReady[3]));
...
void UROS2ActionServer::ProcessAndSendCancelResponse()
{
  ...
  action_msgs__srv__CancelGoal_Response* CancelResponse =
      (action_msgs__srv__CancelGoal_Response*)Action->GetCancelResponse();
...
void UROS2ActionServer::SetDelegates(const FActionCallback& UpdateFeedback, const FActionCallback& UpdateResult,
                                     const FActionCallback& HandleGoal, const FSimpleCallback& HandleCancel,
