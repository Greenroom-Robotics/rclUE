...
void UROS2Publisher::BeginPlay()
{
  ...
  {
    UE_LOG(LogROS2Publisher, Error,
           TEXT("[%s] Cannot locate a NodeSubsystem Actor instance. Initialisation failed."), *GetName());
    return;
  }
...
void UROS2Publisher::Init()
{
  ...
  if (State == UROS2State::Created)
  {
    ...
    {
      UE_LOG(LogROS2Publisher, Error,
             TEXT("[%s (%s)] Topic name not set. Initialisation failed."), *GetName(), *GetOwner()->GetName());
      return;
    }
    ...
    {
      UE_LOG(LogROS2Publisher, Error,
             TEXT("[%s (%s)] Topic Type not set. Initialisation failed."), *GetName(), *GetOwner()->GetName());
      return;
    }
    ...
    RCSOFTCHECK(rcl_publisher_init(&RclPublisher, NodeSubsystem->GetRCLNode(), TopicMessage->GetTypeSupport(),
                                   TCHAR_TO_UTF8(*TopicName), &pub_opt));

    if (bPublishOnTimer)
    {
      GetWorld()->GetTimerManager().SetTimer(TimerHandle, this, &UROS2Publisher::UpdateAndPublishMessage,
                                             1.f / PublicationFrequencyHz, true);
    }
...
void UROS2Publisher::UpdateAndPublishMessage()
{
  ...
  {
    UE_LOG(LogROS2Publisher, Error,
           TEXT("[%s] Update and Publish called when publisher has not been initialised."), *GetName());
    return;
  }
...
void UROS2Publisher::PublishMsg(UROS2GenericMsg* Message, bool async)
{
  ...
  {
    UE_LOG(LogROS2Publisher, Error,
           TEXT("[%s] PublishMsg called when publisher has not been initialised."), *GetName());
    return;
  }
  ...
  if (async)
  {
    if (AsyncPublisherFuture.IsValid())
    {
      ...
      {
        UE_LOG(LogROS2Publisher, Warning,
               TEXT("[%s] Async PublishMsg is still publishing - dropping message."), *GetName());
        return;
      }
    }
