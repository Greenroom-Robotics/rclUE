...
{
  template <typename ROSSequenceT> inline void ResetSequence(ROSSequenceT& out)
  {
    out.data = nullptr;
    out.size = 0;
    out.capacity = 0;
...
namespace ArrayInitialisers
{
  ...
  template <typename T> inline TArray<T> FloatArray(ssize_t elems)
  {
    TArray<T> a;
    a.Init(0.0, elems);
    return a;
  ...
  template <typename T> inline TArray<T> IntArray(ssize_t elems)
  {
    TArray<T> a;
    a.Init(0, elems);
    return a;
...
namespace ROS2MsgToUE
{
  ...
  template <size_t N> inline TArray<uint8> FromArray(const uint8 (&in)[N])
  {
    TArray<uint8> out;
    for (int i = 0; i < N; i++)
    {
  ...
  template <size_t N> inline TArray<float> FromArray(const float (&in)[N])
  {
    TArray<float> out;
    for (int i = 0; i < N; i++)
    {
  ...
  template <size_t N> inline TArray<double> FromArray(const double (&in)[N])
  {
    TArray<double> out;
    for (int i = 0; i < N; i++)
    {
  ...
  template <typename T, typename ROST> inline TArray<T> FromStructSequence(const ROST& in)
  {
    TArray<T> Out;
  ...
  template <typename T, typename ROSSequenceT> inline TArray<T> FromSequence(const ROSSequenceT& in)
  {
    TArray<T> Out;
...
namespace UEToROS2Msg
{
  ...
  template <typename T> inline void Set(const UE::Math::TVector<T>& in, geometry_msgs__msg__Vector3& out)
  {
    out.x = in.X;
    out.y = in.Y;
    out.z = in.Z;
  ...
  template <typename T> inline void Set(const UE::Math::TVector<T>& in, geometry_msgs__msg__Point& out)
  {
    out.x = in.X;
    out.y = in.Y;
    out.z = in.Z;
  ...
  template <typename T> inline void Set(const UE::Math::TVector<T>& in, geometry_msgs__msg__Point32& out)
  {
    out.x = in.X;
    out.y = in.Y;
    out.z = in.Z;
  ...
  template <typename T, typename ROST> inline void SetStructSequence(const TArray<T>& in, ROST& out)
  {
    if (out.data != nullptr)
    {
  ...
  template <typename InT, typename OutT, size_t N> inline void SetSequence(const TArray<InT>& in, OutT (&out)[N])
  {
