...
public:
  virtual void Init() override { geometry_msgs__msg__TwistWithCovariance__init(&twist_with_covariance_msg); }

  virtual void Fini() override { geometry_msgs__msg__TwistWithCovariance__fini(&twist_with_covariance_msg); }
  ...
  void SetMsg(const FROSTwistWithCovariance& Input) { Input.SetROS2(twist_with_covariance_msg); }

  UFUNCTION(BlueprintCallable)
  void GetMsg(FROSTwistWithCovariance& Output) const { Output.SetFromROS2(twist_with_covariance_msg); }

  virtual void* Get() override { return &twist_with_covariance_msg; }
