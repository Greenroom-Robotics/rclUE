...
public:
  virtual void Init() override { geographic_msgs__msg__GeoPose__init(&geo_pose_msg); }

  virtual void Fini() override { geographic_msgs__msg__GeoPose__fini(&geo_pose_msg); }
  ...
  void SetMsg(const FROSGeoPose& Input) { Input.SetROS2(geo_pose_msg); }

  UFUNCTION(BlueprintCallable)
  void GetMsg(FROSGeoPose& Output) const { Output.SetFromROS2(geo_pose_msg); }

  virtual void* Get() override { return &geo_pose_msg; }
