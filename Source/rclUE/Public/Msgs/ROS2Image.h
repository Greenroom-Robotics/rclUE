...
public:
  virtual void Init() override { sensor_msgs__msg__Image__init(&image_msg); }
  ...
  void SetMsg(const FROSImage& Input) { Input.SetROS2(image_msg); }

  UFUNCTION(BlueprintCallable)
  void GetMsg(FROSImage& Output) const { Output.SetFromROS2(image_msg); }

  virtual void* Get() override { return &image_msg; }
