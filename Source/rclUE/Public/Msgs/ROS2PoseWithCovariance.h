...
public:
  virtual void Init() override { geometry_msgs__msg__PoseWithCovariance__init(&pose_with_covariance_msg); }

  virtual void Fini() override { geometry_msgs__msg__PoseWithCovariance__fini(&pose_with_covariance_msg); }
  ...
  void SetMsg(const FROSPoseWithCovariance& Input) { Input.SetROS2(pose_with_covariance_msg); }

  UFUNCTION(BlueprintCallable)
  void GetMsg(FROSPoseWithCovariance& Output) const { Output.SetFromROS2(pose_with_covariance_msg); }

  virtual void* Get() override { return &pose_with_covariance_msg; }
