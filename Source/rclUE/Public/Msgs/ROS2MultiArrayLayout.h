...
public:
  virtual void Init() override { example_interfaces__msg__MultiArrayLayout__init(&multi_array_layout_msg); }

  virtual void Fini() override { example_interfaces__msg__MultiArrayLayout__fini(&multi_array_layout_msg); }
  ...
  void SetMsg(const FROSMultiArrayLayout& Input) { Input.SetROS2(multi_array_layout_msg); }

  UFUNCTION(BlueprintCallable)
  void GetMsg(FROSMultiArrayLayout& Output) const { Output.SetFromROS2(multi_array_layout_msg); }

  virtual void* Get() override { return &multi_array_layout_msg; }
