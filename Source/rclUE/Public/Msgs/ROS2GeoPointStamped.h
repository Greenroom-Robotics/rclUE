...
public:
  virtual void Init() override { geographic_msgs__msg__GeoPointStamped__init(&geo_point_stamped_msg); }

  virtual void Fini() override { geographic_msgs__msg__GeoPointStamped__fini(&geo_point_stamped_msg); }
  ...
  void SetMsg(const FROSGeoPointStamped& Input) { Input.SetROS2(geo_point_stamped_msg); }

  UFUNCTION(BlueprintCallable)
  void GetMsg(FROSGeoPointStamped& Output) const { Output.SetFromROS2(geo_point_stamped_msg); }

  virtual void* Get() override { return &geo_point_stamped_msg; }
