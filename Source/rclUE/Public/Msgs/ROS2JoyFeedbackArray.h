...
public:
  ...
  void SetROS2(ros_msg_c_typename& out_ros_data) const { UEToROS2Msg::SetStructSequence(array, out_ros_data.array); }
};
...
public:
  virtual void Init() override { sensor_msgs__msg__JoyFeedbackArray__init(&joy_feedback_array_msg); }

  virtual void Fini() override { sensor_msgs__msg__JoyFeedbackArray__fini(&joy_feedback_array_msg); }
  ...
  void SetMsg(const FROSJoyFeedbackArray& Input) { Input.SetROS2(joy_feedback_array_msg); }

  UFUNCTION(BlueprintCallable)
  void GetMsg(FROSJoyFeedbackArray& Output) const { Output.SetFromROS2(joy_feedback_array_msg); }

  virtual void* Get() override { return &joy_feedback_array_msg; }
