...
public:
  virtual void Init() override { sensor_msgs__msg__RegionOfInterest__init(&region_of_interest_msg); }

  virtual void Fini() override { sensor_msgs__msg__RegionOfInterest__fini(&region_of_interest_msg); }
  ...
  void SetMsg(const FROSRegionOfInterest& Input) { Input.SetROS2(region_of_interest_msg); }

  UFUNCTION(BlueprintCallable)
  void GetMsg(FROSRegionOfInterest& Output) const { Output.SetFromROS2(region_of_interest_msg); }

  virtual void* Get() override { return &region_of_interest_msg; }
