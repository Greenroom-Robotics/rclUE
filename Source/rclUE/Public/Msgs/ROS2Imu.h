...
public:
  virtual void Init() override { sensor_msgs__msg__Imu__init(&imu_msg); }

  virtual void Fini() override { sensor_msgs__msg__Imu__fini(&imu_msg); }
  ...
  void SetMsg(const FROSImu& Input) { Input.SetROS2(imu_msg); }

  UFUNCTION(BlueprintCallable)
  void GetMsg(FROSImu& Output) const { Output.SetFromROS2(imu_msg); }

  virtual void* Get() override { return &imu_msg; }
