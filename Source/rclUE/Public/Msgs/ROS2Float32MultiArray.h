...
public:
  virtual void Init() override { example_interfaces__msg__Float32MultiArray__init(&float32_multi_array_msg); }

  virtual void Fini() override { example_interfaces__msg__Float32MultiArray__fini(&float32_multi_array_msg); }
  ...
  void SetMsg(const FROSFloat32MultiArray& Input) { Input.SetROS2(float32_multi_array_msg); }

  UFUNCTION(BlueprintCallable)
  void GetMsg(FROSFloat32MultiArray& Output) const { Output.SetFromROS2(float32_multi_array_msg); }

  virtual void* Get() override { return &float32_multi_array_msg; }
